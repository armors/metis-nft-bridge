...
async function mockDeployERC721(originWallet, destWallet, presetTokenIds){
    
    // L1
    const L1Mock = await demo721.connect(originWallet).deploy("L1 721 name", "L1 721 symbol", "ipfs://erc721.io/L1/");
    await L1Mock.deployTransaction.wait();
    console.log(`\n  mockERC721 deployed on L1 @ ${L1Mock.address}`)
    
    // L2
    const L2Mock = await demo721.connect(destWallet).deploy("L2 721 name", "L2 721 symbol", "ipfs://erc721.io/L2/");
    ...
    // L2 mint token
    for(let index = 0; index < presetTokenIds.length; index++) {
        await L2Mock.mint(destWallet.address, presetTokenIds[index]);
    }
    console.log(`\n  mockERC721 mint tokens{ ${presetTokenIds} } on L2`)
...
async function mockDeployERC1155(originWallet, destWallet, presetTokenIds){
    // L1
    ...
    // L1 mint token
    for(let index = 0; index < presetTokenIds.length; index++) {
        await L2Mock.mint(destWallet.address, presetTokenIds[index], 1, [])
    }

    console.log(`\n  mockERC1155 mint tokens{ ${presetTokenIds} } on L2`)
